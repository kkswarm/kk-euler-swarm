 ...
 * \return {*}
 */
tcpDriverNode::tcpDriverNode(std::string name) : Node(name) {
  status_pub_ = this->create_publisher<kkswarm_msgs::msg::KKSwarmState>("/comm", 1000);

  // Create thread to communicate with client.
  boost::thread recvThread(&tcpDriverNode::createSocketServer, this);
  recvThread.detach();
  rclcpp::sleep_for(1s);  // wait for 1s to create socket server.
  ...
  RCLCPP_INFO(this->get_logger(), "Constructor initized.");
}
  ...
  while (true) {
    for (int i = 1; i <= CAR_NUM; i++) {
      ...
      if (car_state_[i].socketfd == 0) {
        ...
        if (time[i] != car_state_[i].header.stamp.nanosec) {
          time[i] = car_state_[i].header.stamp.nanosec;
        } else {
...
int tcpDriverNode::createSocketServer() {
  ...
  while (true) {
    ...
    readThread.detach();
    rclcpp::sleep_for(1s);;  // wait for 1s.
  }
...
void tcpDriverNode::publish2ROS(kkswarm_msgs::msg::KKSwarmState car_state) {
  rclcpp::Rate loop_rate(50);
  car_state.header.stamp = this->now();
...
void tcpDriverNode::subFromROS() {
  ...
  for (int i = 1; i <= CAR_NUM; i++) {
    ...
    auto sub = this->create_subscription<geometry_msgs::msg::Twist>(
        topic, 100,
        [i,this](const geometry_msgs::msg::Twist::SharedPtr msgs) {
          this->velReceviveCallBack(msgs, i);
        });
...
void tcpDriverNode::velReceviveCallBack(
    const geometry_msgs::msg::Twist::SharedPtr msgs, int id) {
...
int main(int argc, char **argv) {
  ...
  auto TDN = std::make_shared<tcpDriverNode>();

  RCLCPP_INFO(TDN->get_logger(), "\033[1;32m----> TCP Communication Started.\033[0m");
