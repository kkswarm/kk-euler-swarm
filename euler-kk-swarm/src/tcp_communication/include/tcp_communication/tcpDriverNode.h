...
class tcpDriverNode: public rclcpp::Node {
 public:
  rclcpp::Publisher<kkswarm_msgs::msg::KKSwarmState>::SharedPtr status_pub_;
  std::vector<rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr> vel_sub_;

  // car_state_[1] means No.1 car's state information.
  kkswarm_msgs::msg::KKSwarmState car_state_[11];
  ...
  tcpDriverNode(std::string name = "tcp_driver_node");
  ~tcpDriverNode();
   ...
   * \return {*}
   */
  void publish2ROS(kkswarm_msgs::msg::KKSwarmState car_state);
   ...
   * \return {*}
   */
  void velReceviveCallBack(const geometry_msgs::msg::Twist::SharedPtr msgs, int id);
